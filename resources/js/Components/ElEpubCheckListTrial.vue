...
import { encryptAES, decryptAES } from '@epub-reader/Libs/utils';
...
const checkListStatus = ref(true);
...
const routeSegment = window.location.pathname.split('/')[2] || '';
const isEbookRoute = computed(() => ['ebook', 'epub-reader'].includes(routeSegment));
let previewLimit = '';
    ...
    >
      ...
      <div class="text-card-title flex w-full items-center justify-center">
        ...
        >
          ...
          <ElInfoButton class="mt-10 mb-5 w-full py-3 md:py-2" @click="checkListStatus = false"
            >觀看電子書</ElInfoButton
          >
        </div>
      </div>
